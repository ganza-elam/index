...
function markReceiptBookletReturned($pdo, $bookletId, $via = 'admin_approve') {
    ensureReceiptTables($pdo);
    $allowed = ['insert_data', 'admin_approve'];
    if (!in_array($via, $allowed, true)) {
        $via = 'admin_approve';
    }
    $stmt = $pdo->prepare("UPDATE receipt_booklets SET returned_at = NOW(), returned_via = ?
        WHERE id = ? AND returned_at IS NULL");
    $stmt->execute([$via, (int) $bookletId]);
    if ($stmt->rowCount() > 0) {
...
function approveReceiptBookletReturn($pdo, $bookletId) {
    if (markReceiptBookletReturned($pdo, (int) $bookletId, 'admin_approve')) {
...
function getReturnedReceiptBookletsPaginated($pdo, $page = 1, $perPage = null, $userId = null) {
    ...
    $sql = "SELECT rb.id AS booklet_id, rb.booklet_no, rb.range_start, rb.range_end,
            rb.returned_at, rb.returned_via,
